...
from .models import Diary, DiarySticker, DiaryTextBox
...
from .swaggerserializer import DiaryGetResponseSerializer, DiaryLinkGetResponseSerializer, \
    DiaryStickerRequestSerializer, \
    DiaryStickerGetResponseSerializer, SwaggerDiaryCreateRequestSerializer, SwaggerDiaryCreateResponseSerializer, \
    DiaryLinkRequestSerializer, DiaryTextBoxPutResponseSerializer, DiarySaveRequestSerializer
...
class DiariesPost(APIView):
    ...
    @swagger_auto_schema(
        ...
        manual_parameters=[
            openapi.Parameter('diary_id', openapi.IN_QUERY, description="Diary ID", type=openapi.TYPE_INTEGER,
                              required=False), ]
    )
    def post(request):
        client_ip = request.META.get('REMOTE_ADDR', None)
        ...
        if calendar_id is None:
            ...
            if calendar_serializer.is_valid():
                ...
                logger.warning(
                    f'WARNING {client_ip} {current_time} POST api/v1/diaries//DiriesPost 400 {calendar_serializer.errors}')
            ...
            if diary_serializer.is_valid():
                ...
                if diary_update_serializer.is_valid():
                    ...
                    logger.error(
                        f'ERROR {client_ip} {current_time} POST api/v1/diaries//DiriesPost 400 {diary_serializer.errors}')
        ...
        if calendar_id is not None:
            ...
            if diary_serializer.is_valid():
                ...
                if diary_update_serializer.is_valid():
                    ...
                    logger.error(
                        f'ERROR {client_ip} {current_time} POST api/v1/diaries//DiriesPost 400 {diary_update_serializer.errors}')
                    return Response(status=status.HTTP_400_BAD_REQUEST, data={'error': 'snsLink가 유효하지 않습니다.'})
            else:
                logger.error(
                    f'ERROR {client_ip} {current_time} POST api/v1/diaries//DiriesPost 400 {diary_serializer.errors}')
...
class DiariesSave(APIView):
    @swagger_auto_schema(operation_description="일기 최종 저장",
                         ...
                         request_body=DiarySaveRequestSerializer,
    ...
    def put(self, request):  # 캘린더 아디랑
        ...
        if diary_id is None:
            logger.warning(
                f'WARNING {client_ip} {current_time} PUT api/v1/diaries//DiariesSave 400 diary does not exist')
            return Response({"error": "diary does not exist"}, status=status.HTTP_404_NOT_FOUND)
        if request.data is None:
            logger.warning(
                f'WARNING {client_ip} {current_time} POST api/v1/diaries//DiariesSave 400 diary does not exist')
        ...
        diary_instance.save()
        saved_data = request.data.get('saved_data', {})
        print(saved_data)
        textboxs_data = saved_data.get('textboxs', [])
        print(textboxs_data)
        stickers_data = saved_data.get('stickers', [])
        print(stickers_data)
        try:
            for sticker_data in stickers_data:
                ...
                if sticker_serializer.is_valid():
                    ...
                    logger.error(
                        f'ERROR {client_ip} {current_time} PUT api/v1/diaries//DiariesSave 400 {sticker_serializer.errors}')
                    return Response({"error": "stickerbox error"}, status=status.HTTP_404_NOT_FOUND)
        except ObjectDoesNotExist:
            logger.error(
                f'ERROR {client_ip} {current_time} PUT api/v1/diaries//DiariesSave 400 {sticker_serializer.errors}')
            return Response({"error": "stickerbox error"}, status=status.HTTP_404_NOT_FOUND)
        try:
            for textbox_data in textboxs_data:
                ...
                if textbox_serializer.is_valid():
                    ...
                    logger.error(
                        f'ERROR {client_ip} {current_time} PUT api/v1/diaries//DiariesSave 400 {textbox_serializer.errors}')
                    return Response({"error": "textbox error"}, status=status.HTTP_404_NOT_FOUND)
        except ObjectDoesNotExist:
            logger.error(
                f'ERROR {client_ip} {current_time} PUT api/v1/diaries//DiariesSave 400 {textbox_serializer.errors}')
            return Response({"error": "textbox error"}, status=status.HTTP_404_NOT_FOUND)
        ...
        return Response({'code': 'D001', 'status': '201', 'message': '일기장 업데이트 성공!'}, status=status.HTTP_200_OK)
...
class DiaryManager(APIView):
    ...
    def get(self, request):
        ...
        try:
            ...
            logger.warning(
                f'WARNING {client_ip} {current_time} GET api/v1/diaries//DiaryManager 200 diary snsLink does not exist')
    ...
    def post(self, request, format=None):
        ...
        try:
            ...
            if not top_keywords:
                ...
                logger.warning(
                    f'WARNING {client_ip} {current_time} POST api/v1/diaries//DiaryStickerManager 200 keywords are not created')
                return Response(response_data, status=200)
            ...
            logger.info(
                f'INFO {client_ip} {current_time} POST api/v1/diaries//DiaryStickerManager 201 images are created')
            return Response(response_data, status=201)
        except NoCredentialsError:
            logger.error(
                f'ERROR {client_ip} {current_time} POST api/v1/diaries//DiaryStickerManager 500 AWS credentials not available.')
